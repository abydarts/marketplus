...
class twocheckout_gate extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->library(array('bse_tec','Twocheckout'));
	...
	*/
	...
	{
		$settings = $this->bse_tec->CI->Settings_model->get_settings('twocheckout');

		if ($this->bse_tec->CI->input->post('order_number'))
		{
			foreach ($this->bse_tec->CI->input->post() as $k => $v)
			{
				$response[$k] = $v;
			}
		}
		elseif ($this->bse_tec->CI->input->get('order_number'))
		{
			foreach ($this->bse_tec->CI->input->get() as $k => $v)
			{
				$response[$k] = $v;
			}
		}
		...
		{            
			...
			// set a confirmed flag in the bsetec payment property
